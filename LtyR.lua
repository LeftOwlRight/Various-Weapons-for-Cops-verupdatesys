...
Hooks:Add("MenuManagerBuildCustomMenus", "MenuManagerBuildCustomMenusRandomWeapon", function(menu_manager, nodes)
	...
	AutoMenuBuilder:create_menu_from_table(nodes, VariousWeaponForCops.settings, "various_weapon_for_cops", "blt_options",VariousWeaponForCops.values)
end)
...
-- danger or not
	if VariousWeaponForCops.settings.Dangerous_mode then
        ...
        local deck_switch = {
                    [15] = function()  --Anarchist
                        if self._unit:base()._tweak_table == "city_swat" or self._unit:base()._tweak_table == "fbi_swat" or self._unit:base()._tweak_table == "fbi_heavy_swat" then
